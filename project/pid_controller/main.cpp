  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          // Calculate the desired steer using the starting and ending points of the tracjectory
		  if ((x_points.size() > 0) && (y_points.size() > 0)){
           	delta_x = x_points[x_points.size() - 1] - x_points[0];
            delta_y = y_points[y_points.size() - 1] - y_points[0];
